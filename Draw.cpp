...
{
    drawGrille(gameEngine,windowGiven);
}

void Draw::drawGrille(GameEngine &gameEngine, sf::RenderWindow &windowGiven)
{
...
void Draw::drawSquare(Square &square, sf::RenderWindow &window, int offset)
{
    ...
    int y = ((Config::SQUARESIZE + (Config::THICKNESS * 2)) * (position.second / 2)) + offset; // /2 car on retire une ligne sur 2
